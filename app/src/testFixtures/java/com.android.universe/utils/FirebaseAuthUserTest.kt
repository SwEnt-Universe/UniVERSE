 ...
 * This class performs a "Hard Wipe" of the emulator data before every test.
 * This ensures a clean state but prevents parallel test execution.
 */
open class FirebaseAuthUserTest(private val isRobolectric: Boolean = true) {
    ...
    /**
     * Wipes ALL users from the Auth Emulator via REST API.
     * Throws an exception if cleanup fails to prevent tests running on dirty data.
     */
    private fun clearAuthEmulator() {
        val projectId = FirebaseApp.getInstance().options.projectId
        val host = if (isRobolectric) "127.0.0.1" else "10.0.2.2"
        val url = URL("http://$host:${FirebaseEmulator.AUTH_PORT}/emulator/v1/projects/$projectId/accounts")

        sendDeleteRequest(url, "Auth")
    }

    /**
     * Wipes ALL data from the Firestore Emulator via REST API.
     * This is atomic and much faster than deleting documents one by one.
     */
    private fun clearFirestoreEmulator() {
        val projectId = FirebaseApp.getInstance().options.projectId
        val host = if (isRobolectric) "127.0.0.1" else "10.0.2.2"
        // The endpoint to wipe the default database
        val url = URL("http://$host:${FirebaseEmulator.FIRESTORE_PORT}/emulator/v1/projects/$projectId/databases/(default)/documents")

        sendDeleteRequest(url, "Firestore")
    }

    /**
     * Helper to send DELETE requests to Emulator REST endpoints.
     * Fails FAST if the emulator is unreachable or returns an error.
     */
    private fun sendDeleteRequest(url: URL, serviceName: String) {
        ...
        try {
            ...
            throw RuntimeException("CRITICAL: Could not connect to $serviceName emulator for cleanup. Is it running?", e)
        } finally {
            conn?.disconnect()
        }
    }
    ...
    /**
     * Creates a test user. Since we wipe the DB, we can be more relaxed about IDs,
     * but using random IDs is still a good habit.
     */
