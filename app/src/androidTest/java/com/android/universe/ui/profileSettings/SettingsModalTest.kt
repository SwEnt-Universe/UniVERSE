...
class SettingsModalTest {
	...
	fun emailModal_allowsEditing_and_buttonsVisible() {
		...
		composeTestRule.onNodeWithTag(SettingsTestTags.MODAL_TITLE, useUnmergedTree = true).assertIsDisplayed()
		composeTestRule.onNodeWithTag(SettingsTestTags.MODAL_CANCEL_BUTTON, useUnmergedTree = true).assertIsDisplayed()
		composeTestRule.onNodeWithTag(SettingsTestTags.MODAL_SAVE_BUTTON, useUnmergedTree = true).assertIsDisplayed()

		val field = composeTestRule.onNode(
			hasSetTextAction() and androidx.compose.ui.test.hasTestTag(SettingsTestTags.EMAIL_FIELD),
	...
	fun passwordModal_allowsEditing() {
		...
		val field = composeTestRule.onNode(
			hasSetTextAction() and androidx.compose.ui.test.hasTestTag(SettingsTestTags.PASSWORD_FIELD),
			useUnmergedTree = true
		)
		field.performClick()
		field.performTextClearance()
	...
	fun firstNameModal_allowsEditing() {
		...
		val field = composeTestRule.onNode(
			hasSetTextAction() and androidx.compose.ui.test.hasTestTag(SettingsTestTags.FIRST_NAME_FIELD),
			useUnmergedTree = true
		)
		field.performClick()
		field.performTextClearance()
	...
	fun lastNameModal_allowsEditing() {
		...
		val field = composeTestRule.onNode(
			hasSetTextAction() and androidx.compose.ui.test.hasTestTag(SettingsTestTags.LAST_NAME_FIELD),
			useUnmergedTree = true
		)
		field.performClick()
		field.performTextClearance()
	...
	fun descriptionModal_multiline_and_errorShown() {
		var last = ""
		val ui = sampleSettingsState(showModal = true, field = "description").copy(
			tempValue = "",
			modalError = "Too short"
		)
		renderWith(ui, onUpdateTemp = { k, v -> if (k == "tempValue") last = v })
		...
		val field = composeTestRule.onNode(
			hasSetTextAction() and androidx.compose.ui.test.hasTestTag(SettingsTestTags.DESCRIPTION_FIELD),
			useUnmergedTree = true
		)
		field.performClick()
		field.performTextClearance()
	...
	fun countryModal_dropdown_pickUpdates_andCloses() {
		...
		val ui = sampleSettingsState(showModal = true, field = "country").copy(
			tempValue = "",
		...
		composeTestRule
			.onNodeWithTag("${SettingsTestTags.COUNTRY_OPTION_PREFIX}$firstOption", useUnmergedTree = true)
			.performClick()
	...
	fun countryModal_tappingField_togglesExpansion() {
		...
		renderWith(
			ui,
			onToggleCountryDropdown = { expanded -> lastExpanded = expanded }
		)
		...
		// We only assert that we got a toggle request to 'true' (actual visual expansion is owned by upstream state)
	...
	fun dateModal_inputsUpdate_and_showErrors() {
		var d = ""; var m = ""; var y = ""
		val ui = sampleSettingsState(showModal = true, field = "date").copy(
			tempDay = "",
			tempMonth = "",
			tempYear = "",
		...
		day.performClick(); day.performTextClearance(); day.performTextInput("07")

		val month = composeTestRule.onNodeWithTag(SettingsTestTags.MONTH_FIELD, useUnmergedTree = true)
		month.performClick(); month.performTextClearance(); month.performTextInput("12")

		val year = composeTestRule.onNodeWithTag(SettingsTestTags.YEAR_FIELD, useUnmergedTree = true)
		year.performClick(); year.performTextClearance(); year.performTextInput("1999")
	...
	fun saveAndCancel_invokeCallbacks() {
		...
		renderWith(
			ui,
			onCloseModal = { closed = true },
			onSaveModal = { saved = true }
		)

		composeTestRule.onNodeWithTag(SettingsTestTags.MODAL_SAVE_BUTTON, useUnmergedTree = true).performClick()
		composeTestRule.onNodeWithTag(SettingsTestTags.MODAL_CANCEL_BUTTON, useUnmergedTree = true).performClick()
