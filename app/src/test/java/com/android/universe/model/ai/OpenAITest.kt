...
class OpenAITest {
	...
	fun testOpenAIRequestAndPersistence() = runBlocking {
		...
		assert(saved.all { it.id.isNotBlank() }) {
			"Events saved in repository must have assigned IDs"
		}

		// 3. Verify persistence
		val repoEvents = repo.getAllEvents()
		assert(repoEvents.size == saved.size) {
			"Repository event count mismatch"
		}

		saved.forEach { e ->
			assert(repoEvents.any { it.id == e.id }) {
				"Event with id ${e.id} not found in repository"
			}
		}
	}
}
