...
class EventViewModelTest {
    ...
    fun firstEventIsMappedCorrectly() = runTest {
        ...
        assertEquals(
            "Join us for a casual 5km run around the lake followed by coffee.",
            firstEvent.description
        )
        assertEquals("15 Oct 07:30 AM", firstEvent.date)
        assertEquals(listOf("Sport", "Outdoor"), firstEvent.tags)
    ...
    fun eventWithNoDescriptionHasEmptyString() = runTest {
        val hackathonEvent =
            viewModel.eventsState.value.first { it.title == "Tech Hackathon 2025" }
    ...
    fun eventsWithMoreThanThreeTagsAreCropped() = runTest {
        ...
        val megaTagEvent =
            viewModel.eventsState.value.first { it.title == "Mega Tag Event" }

        assertEquals(3, megaTagEvent.tags.size)
        assertEquals(listOf("Tech", "AI", "Innovation"), megaTagEvent.tags)
    }
