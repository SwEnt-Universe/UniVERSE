...
class TagItemTest {
    ...
    fun setUp(isSelectable: Boolean, isAlreadySelected: Boolean, onSelect: (Tag) -> Unit, onDeSelect: (Tag) -> Unit) {
        composeTestRule.setContentWithStubBackdrop { TagItem(tag = tag, isSelectable = isSelectable, isAlreadySelected = isAlreadySelected, onSelect = onSelect, onDeSelect = onDeSelect) }
    }
    ...
    fun notSelectedTagOnlyDisplayText(){
        ...
        composeTestRule.onNodeWithTag(TagItemTestTag.ICON, useUnmergedTree = true).assertIsNotDisplayed()
    }
    ...
    fun tagCanBeSelected(){
        ...
        setUp(isSelectable = true, isAlreadySelected = false, onSelect = {tag -> tags.add(tag)}, onDeSelect = {})
    ...
    fun tagCanBeDeSelected(){
        ...
        setUp(isSelectable = true, isAlreadySelected = true, onSelect = {}, onDeSelect = {tag -> tags.remove(tag)})
        composeTestRule.onNodeWithTag(TagItemTestTag.BUTTON).performClick()
        composeTestRule.onNodeWithTag(TagItemTestTag.ICON, useUnmergedTree = true).assertIsNotDisplayed()
