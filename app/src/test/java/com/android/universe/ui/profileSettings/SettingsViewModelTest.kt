...
class SettingsViewModelTest {
  ...
  fun saveModal_commitsSelectedInterestTags_replacingCategory() =
      runTest(testDispatcher) {
        ...
        viewModel.openModal("interest_tags")
        val picks = tagsInterest.take(2)
        picks.forEach { tag -> viewModel.addTag(tag) }

        viewModel.saveModal("emma")
        advanceUntilIdle()

        val names = viewModel.uiState.value.selectedTags.map(Tag::name)
        assertEquals(picks.sorted(), names.filter { it in tagsInterest }.sorted())
      }
