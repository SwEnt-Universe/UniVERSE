...
class SettingsViewModelTest {
  ...
  // loadUser Tests
  @Test
  fun `loadUser populates UiState from repository`() = runTest {
  ...
  @Test
  fun `loadUser sets errorMsg on repository failure`() = runTest {
  ...
  // clearErrorMsg Tests
  @Test
  fun `clearErrorMsg resets errorMsg`() = runTest {
    coEvery { mockRepo.getUser("0") } throws NoSuchElementException("No user found")
    every { UserRepositoryProvider.repository } returns mockRepo
    val viewModel = SettingsViewModel(UserRepositoryProvider)
  ...
  // updateTemp Tests
  @Test
  fun `updateTemp updates tempValue and clears modalError`() = runTest {
  ...
  @Test
  fun `updateTemp updates tempDay and clears tempDayError`() = runTest {
  ...
  @Test
  fun `updateTemp ignores invalid key`() = runTest {
  ...
  // openModal Tests
  @Test
  fun `openModal prefills temp for text fields`() = runTest {
    viewModel.loadUser("0")
    advanceUntilIdle()
  ...
  @Test
  fun `openModal prefills date triplet`() = runTest {
    viewModel.loadUser("0")
    advanceUntilIdle()
  ...
  // closeModal Tests
  @Test
  fun `closeModal resets modal state`() = runTest {
    viewModel.openModal("date")
    viewModel.updateTemp("tempDay", "32")
    viewModel.updateTemp("tempMonth", "13")
    viewModel.updateTemp("tempYear", "1800")
  ...
  @Test
  fun `toggleCountryDropdown updates showCountryDropdown`() = runTest {
  ...
  // saveModal Tests
  @Test
  fun `saveModal updates email when valid`() = runTest {
    viewModel.loadUser("0")
    advanceUntilIdle()
    viewModel.openModal("email")
  ...
  @Test
  fun `saveModal sets modalError for invalid email`() = runTest {
    viewModel.loadUser("0")
    advanceUntilIdle()
    viewModel.openModal("email")
    viewModel.updateTemp("tempValue", "invalid")
  ...
  @Test
  fun `saveModal updates password when valid`() = runTest {
    viewModel.loadUser("0")
    advanceUntilIdle()
    viewModel.openModal("password")
  ...
  @Test
  fun `saveModal sets modalError for invalid password`() = runTest {
    viewModel.loadUser("0")
    advanceUntilIdle()
    viewModel.openModal("password")
  ...
  @Test
  fun `saveModal updates firstName when valid`() = runTest {
    viewModel.loadUser("0")
    advanceUntilIdle()
    viewModel.openModal("firstName")
  ...
  @Test
  fun `saveModal sets modalError for invalid firstName`() = runTest {
    viewModel.loadUser("0")
    advanceUntilIdle()
    viewModel.openModal("firstName")
  ...
  @Test
  fun `saveModal updates lastName when valid`() = runTest {
    viewModel.loadUser("0")
    advanceUntilIdle()
    viewModel.openModal("lastName")
  ...
  @Test
  fun `saveModal sets modalError for invalid lastName`() = runTest {
    viewModel.loadUser("0")
    advanceUntilIdle()
    viewModel.openModal("lastName")
    viewModel.updateTemp("tempValue", "")
  ...
  @Test
  fun `saveModal updates description when valid`() = runTest {
    viewModel.loadUser("0")
    advanceUntilIdle()
    viewModel.openModal("description")
  ...
  @Test
  fun `saveModal sets modalError for invalid description`() = runTest {
    viewModel.loadUser("0")
    advanceUntilIdle()
    viewModel.openModal("description")
  ...
  @Test
  fun `saveModal updates country when valid`() = runTest {
    viewModel.loadUser("0")
    advanceUntilIdle()
    viewModel.openModal("country")
  ...
  @Test
  fun `saveModal sets modalError for empty country`() = runTest {
    viewModel.loadUser("0")
    advanceUntilIdle()
    viewModel.openModal("country")
    viewModel.updateTemp("tempValue", "")
  ...
  @Test
  fun `saveModal updates date when valid`() = runTest {
    viewModel.loadUser("0")
    advanceUntilIdle()
    viewModel.openModal("date")
  ...
  @Test
  fun `saveModal sets date errors for invalid date`() = runTest {
    viewModel.loadUser("0")
    advanceUntilIdle()
    viewModel.openModal("date")
    viewModel.updateTemp("tempDay", "32")
  ...
  // saveProfile Tests
  @Test
  fun `saveProfile updates repository when all valid`() = runTest {
    viewModel.loadUser("0")
    advanceUntilIdle()
    viewModel.openModal("firstName")
    viewModel.updateTemp("tempValue", "Emilia")
  ...
  @Test
  fun `saveProfile updates email in Firebase when changed`() = runTest {
    val fakeAuth = mockk<FirebaseAuth>(relaxed = true)
    every { FirebaseAuth.getInstance() } returns fakeAuth
    every { fakeAuth.currentUser } returns mockFirebaseUser
  ...
  @Test
  fun `saveProfile does not update email if unchanged`() = runTest {
    val fakeAuth = mockk<FirebaseAuth>(relaxed = true)
    every { FirebaseAuth.getInstance() } returns fakeAuth
    every { fakeAuth.currentUser } returns mockFirebaseUser
  ...
  @Test
  fun `saveProfile updates password in Firebase when provided`() = runTest {
    val fakeAuth = mockk<FirebaseAuth>(relaxed = true)
    every { FirebaseAuth.getInstance() } returns fakeAuth
    every { fakeAuth.currentUser } returns mockFirebaseUser
  ...
  @Test
  fun `saveProfile does not update password if empty`() = runTest {
    val fakeAuth = mockk<FirebaseAuth>(relaxed = true)
    every { FirebaseAuth.getInstance() } returns fakeAuth
    every { fakeAuth.currentUser } returns mockFirebaseUser
