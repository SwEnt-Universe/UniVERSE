...
/**
 * Tests for the [CustomTextField] composable. This test suite verifies display logic, icon
 * visibility, password masking, and validation state rendering.
 */
...
class CustomTextFieldTest {
  ...
  fun customTextField_displaysLabel() {
    composeTestRule.setContent {
      UniverseTheme {
        CustomTextField(label = LABEL, placeholder = PLACEHOLDER, value = "", onValueChange = {})
      }
    }
  ...
  fun customTextField_displaysValue_andHidesPlaceholder() {
    composeTestRule.setContent {
      UniverseTheme {
        CustomTextField(label = LABEL, placeholder = PLACEHOLDER, value = VALUE, onValueChange = {})
      }
    }
  ...
  fun customTextField_onToggleVisibility_isCalled_whenIconIsClicked() {
    ...
    composeTestRule.runOnIdle { assert(toggled) { "onToggleVisibility was not called" } }
  }
