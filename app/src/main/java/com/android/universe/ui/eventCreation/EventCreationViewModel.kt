...
class EventCreationViewModel(
   ...
   fun setEventDay(day: String) {
    ...
    fun saveEvent(location: Location){
        viewModelScope.launch {
            try {
                val id = eventRepository.getNewID()
                val date = eventCreationUiState.value.day + "/" +
                        eventCreationUiState.value.month + "/" +
                        eventCreationUiState.value.year
                val time = eventCreationUiState.value.hour + ":" +
                        eventCreationUiState.value.minute
                ...
                val currentUser = Firebase.auth.currentUser?.uid ?: return@launch

                val creator = userRepository.getUser(currentUser)
                val event = Event(
                    id = id,
