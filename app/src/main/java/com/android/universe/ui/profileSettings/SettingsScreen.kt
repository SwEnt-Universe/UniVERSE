...
fun SettingsScreen(
	...
	SettingsScreenContent(
		uiState = uiState,
		onBack = onCancel,
		onSave = { viewModel.saveProfile(username, onSaveSuccess) },
		...
		onSaveModal = viewModel::saveModal
	)
}
...
private fun InterestsSection(
	...
	@Composable
	fun chipsLine(label: String, names: List<String>, tag: String) = EditableField(
		label = label,
		value = names.joinToString(", ").take(30) +
				if (names.joinToString().length > 30) "..." else "",
		testTag = tag
	) {
		// incoming tags are "..._button"; strip to field key for modal
	...
	Column(
		...
		when (uiState.currentField) {
			"email", "password", "firstName", "lastName", "description" -> {
				val value = when (uiState.currentField) {
					"email" -> uiState.email
				...
				val label = when (uiState.currentField) {
					"email" -> "Email"
				...
				OutlinedTextField(
					...
					modifier = Modifier
						...
						.onFocusChanged {
							if (it.isFocused) {
								// re-emit current value to clear per-field error if any
 ...
 * ========================================================= */
private fun sampleSettingsState(
