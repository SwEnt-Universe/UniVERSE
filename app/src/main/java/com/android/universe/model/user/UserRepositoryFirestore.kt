...
class UserRepositoryFirestore(
  ...
  /**
   * Retrieves a list of user profiles that follow the current user.
   *
   * @param currentUserId The unique ID of the current user.
   * @return A list of [UserProfile] objects representing the users that follow the current user.
   */
  override suspend fun getFollowers(currentUserId: String): List<UserProfile> {
    val user = getUser(currentUserId)
    val followers = mutableListOf<UserProfile>()
    for (followerId in user.followers) {
      val followerProfile = getUser(followerId)
  ...
  /**
   * Retrieves a list of user profiles that the current user is following.
   *
   * @param currentUserId The unique ID of the current user.
   * @return A list of [UserProfile] objects representing the users that the current user is
   *   following.
   */
  override suspend fun getFollowing(currentUserId: String): List<UserProfile> {
    val user = getUser(currentUserId)
    val following = mutableListOf<UserProfile>()
    for (followingId in user.following) {
      val followingProfile = getUser(followingId)
  ...
  /**
   * Retrieves a list of user profiles recommended for the current user to follow.
   *
   * The recommendations are generated by filtering out users that the current user already follows,
   * and then randomly selecting up to 30 users from the remaining pool.
   *
   * @param currentUserId The unique ID of the current user.
   * @return A list of [UserProfile] objects representing recommended users to follow.
   */
  override suspend fun getFollowRecommendations(currentUserId: String): List<UserProfile> {
    ...
    val recommendations =
        allUsers.filter { user ->
          user.uid != currentUserId && !currentUser.following.contains(user.uid)
        }
    return recommendations.shuffled().take(30)
  }
}
