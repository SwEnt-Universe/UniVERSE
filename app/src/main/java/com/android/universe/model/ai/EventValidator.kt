...
object EventValidator {
	...
	fun validate(dto: EventDTO) {
		...
		require(dto.description.isNotBlank()) {
			"Event description cannot be empty."
		}

		requireNotNull(dto.location) {
			"Event location is missing."
		}

		require(dto.location.latitude in -90.0..90.0) {
			"Invalid latitude: ${dto.location.latitude}"
		}
		...
		runCatching { LocalDateTime.parse(dto.date) }
			.getOrElse {
				throw IllegalArgumentException("Invalid date format: ${dto.date}")
			}
	}
}
