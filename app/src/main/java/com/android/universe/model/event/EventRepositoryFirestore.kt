...
class EventRepositoryFirestore(private val db: FirebaseFirestore): EventRepository {
    ...
    private fun mapToUserProfile(map: Map<String, Any?>): UserProfile {
        return UserProfile(
            ...
            tags = (map["tags"] as? List<Number>)?.map { ordinal -> Tag.entries[ordinal.toInt()] }?.toSet() ?: emptySet()
        )
    }
    ...
    private fun documentToEvent(doc: DocumentSnapshot): Event {
        return try{
            Event(
                ...
                creator = mapToUserProfile(doc.get("creator") as? Map<String, Any?> ?: throw Exception("Creator data missing")),
                participants = (doc.get("participants") as? List<Map<String, Any?>>)?.map { mapToUserProfile(it) }?.toSet() ?: emptySet()
            )
        } catch (e: Exception) {
    ...
    override suspend fun getAllEvents(): List<Event> {
        ...
        val querySnapshot =
            db.collection(EVENTS_COLLECTION_PATH).get().await()
